/**
 * Utility function to calculate daily macro targets using the Mifflin-St Jeor formula
 * based on user biometrics (weight, height, age, gender, goal, activity level).
 */
export const calculateMacroTargets = (user) => {
  if (user?.dailyTargets) {
    const dt = user.dailyTargets;
    return {
      targetCalories: dt.calories || 2200,
      targetProtein: dt.protein || 140,
      targetCarbs: dt.carbs || 250,
      targetFats: dt.fats || 60,
      calories: dt.calories || 2200,
      protein: dt.protein || 140,
      carbs: dt.carbs || 250,
      fats: dt.fats || 60
    };
  }
